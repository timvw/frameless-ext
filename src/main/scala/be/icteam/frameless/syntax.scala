...
package object syntax {
  ...
  implicit class TypedDatasetOps[T](val tds: TypedDataset[T]) {
  ...
  implicit class TypedColumnOps[T, A](tc: TypedColumn[T, A]) {
    ...
    def collectList(
                     implicit ta: TypedEncoder[A]): TypedAggregate[T, Vector[A]] = {
    ...
    def sum[Out](implicit summable: CatalystSummable[A, Out],
                 aencoder: TypedEncoder[A],
                 oencoder: TypedEncoder[Out]): TypedAggregate[T, Out] = {
    ...
    def avg[Out](implicit averageable: CatalystAverageable[A, Out],
                 oencoder: TypedEncoder[Out]): TypedAggregate[T, Out] = {
      frameless.functions.aggregate.avg[A, T, Out](tc)
    }

    def variance[Out](implicit averageable: CatalystAverageable[A, Out],
                      oencoder: TypedEncoder[Out]): TypedAggregate[T, Out] = {
      frameless.functions.aggregate.avg[A, T, Out](tc)
    }

    def variance(
                  implicit variance: CatalystVariance[A]): TypedAggregate[T, Double] = {
    ...
    def stddevPop(implicit ev: CatalystCast[A, Double])
    : TypedAggregate[T, Option[Double]] = {
      frameless.functions.aggregate.stddevPop[A, T](tc)
    }

    def stddevSamp(implicit ev: CatalystCast[A, Double])
    : TypedAggregate[T, Option[Double]] = {
    ...
    def corr[B](column2: TypedColumn[T, B])(
    ...
    def covarPop[B](column2: TypedColumn[T, B])(
      implicit i0: CatalystCast[A, Double],
      i1: CatalystCast[B, Double]): TypedAggregate[T, Option[Double]] = {
      frameless.functions.aggregate.covarPop[A, B, T](tc, column2)
    }

    def covarSamp[B](column2: TypedColumn[T, B])(
      implicit i0: CatalystCast[A, Double],
      i1: CatalystCast[B, Double]): TypedAggregate[T, Option[Double]] = {
      frameless.functions.aggregate.covarSamp[A, B, T](tc, column2)
    }

    def kurtosis(implicit ev: CatalystCast[A, Double])
    : TypedAggregate[T, Option[Double]] = {
